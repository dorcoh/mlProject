...
def generateClassifiers(xTrain, yTrain,xTest,yTest):
    """
...
def predictSample(xTest,yTest,docId,clfDict,setsDict, misDict):
    """
    ...
    for i in range(1,5):
        result.append(bool(int(maxKey.split(',')[i][2])))
...
def superbClassifier(xTrain,yTrain,xTest,yTest):
    """
    ...
    clfList, allSet = generateClassifiers(xTrain, yTrain, xTest,yTest)
    predScores = []
    # predict label for each doument (4 paragraphs)
    for doc in range(0, len(xTest)/4):
        docScores = predictSample(xTest,yTest,doc,clfList,allSet,misDict)
        for i in range(0,4):
    ...
    # print results
    printRes('baseline',baselinePred,yTest.score)
    printRes('superb',predScores,yTest.score)
...
if __name__ == '__main__':
    ...
    superbClassifier(xTrain,yTrain,xTest,yTest)
    # assumption checking
